...
function FirmenDropdown({
  ...
  const gefilterteOptionen = optionen.filter((o) =>
    o.name.toLowerCase().includes(suche.trim().toLowerCase()),
  );

  // Suchfeld beim Schließen leeren.
  useEffect(() => {
    if (!offen) setSuche("");
  }, [offen]);
        ...
        <div className="absolute right-0 z-20 mt-2 w-72 max-w-[85vw] rounded-lg border border-border bg-background p-2 shadow-lg">
          ...
          <div className="relative mb-2">
            ...
            <input
              ...
              autoFocus
              className="w-full rounded-md border border-border bg-background py-1.5 pl-8 pr-2 text-sm outline-none focus:border-signal focus:ring-1 focus:ring-signal"
            />
          </div>
